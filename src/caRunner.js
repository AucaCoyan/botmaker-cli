...
exports.runner = async (code, context, helpers, fulfill, token, filename) => {
  ...
  const resolveSalesforceContext = () =>
    rp({
      method: 'POST',
  ...
  const resolveGoogleSheetContext = () =>
    rp({
      method: 'POST',
  ...
  const resolveGoogleCalendarContext = () =>
    rp({
      method: 'POST',
  ...
  try {
    ...
    const connectRedis = () => {
      throw Error(
        'unimplemented!\n   due to compilation problems, i decided to remove support for the redis client until further notice',
      );
    };
    ...
    const db = {
      get: async (k) => {
        const lkd22lakd2 = await fetch(
          'https://us-west1-m-infra.cloudfunctions.net/code_action_pair/get',
          { headers: { o: 'api', k, t: token } },
        );
      ...
      del: async (k) => {
        await fetch(
          'https://us-west1-m-infra.cloudfunctions.net/code_action_pair/del',
          { headers: { o: 'api', k, t: token }, method: 'POST' },
        );
      },
      set: async (k, v) => {
        await fetch(
          'https://us-west1-m-infra.cloudfunctions.net/code_action_pair/set',
          {
            headers: { o: 'api', k, t: token, 'Content-Type': 'text/plain' },
            method: 'POST',
            body: v,
          },
        );
    ...
    const user = {
      set: (k, v) => {
        ...
        if (v && v.length > MAX_USER_VAR_SIZE)
          throw new Error(
            `Trying to set a very long value for user[${k}]; value has ${v.length} length. The max allowed size is: ${MAX_USER_VAR_SIZE}`,
          );

        setVars.add(k);
        if (setVars.size > MAX_USER_VAR_COUNT)
          throw new Error(
            `Too many user variables set in this action. Max allowed count is: ${MAX_USER_VAR_COUNT}`,
          );
    ...
    const promiseRetry = (func, ms = 100, maxRetries = 2, lastError) =>
      new Promise((resolve, reject) => {
        if (maxRetries === 0) reject(lastError);
        else
          func.then(resolve).catch((e) => {
            setTimeout(
              () =>
                promiseRetry(func, ms, maxRetries - 1, e)
                  .then(resolve)
                  .catch(reject),
              ms,
            );
    ...
    const result = {
      ...
      image: (url, MESSAGE) =>
        resultState.say.push({ IMAGES_URLS: [url], MESSAGE }),
      video: (url) => resultState.say.push({ VIDEOS_URLS: [url] }),
      file: (url, MESSAGE) =>
        resultState.say.push({ FILES_URLS: [url], MESSAGE }),
      ...
      buttonsBuilder: () => {
        return {
          ...
          addClientActionButton: function (
            name,
            clientActionNameOrId,
            parameters,
            synonyms,
          ) {
            let action = {
      ...
      appleBusinesChatCustomInteractiveMessage: (nextIntentName) => {
        return {
          ...
          setCustom: function (bid, appId, appName, URL) {
      ...
      appleBusinesChatApplePay: (nextIntentName, problemsIntentName) => {
        return {
          ...
          setEndpoints: function (
            ...
            this.message.ABC_PAY_ENDPOINTS = {
              paymentGatewayUrl,
              fallbackUrl,
              orderTrackingUrl,
          ...
          addLineItem: function (amount, label) {
            ...
            this.message.ABC_PAY_PAYMENT = {
              ...this.message.ABC_PAY_PAYMENT,
              items: this.message.ABC_PAY_PAYMENT.items.concat(itm),
            };
      ...
      appleBusinesChatForms: (
        nextIntentName,
        showSummary = false,
        splashImageRefIndex = -1,
        splashHeader = null,
        splashText = null,
        splashButtonTitle = null,
      ) => {
        return {
          message: {
            ABC_IMAGES: [],
          ...
          addDatePickerPage: function (
            ...
            return this.addPageImpl(
              ...
              nextPageId,
              isSubmitForm,
              { dateFormat, startDate, minimumDate, maximumDate, labelText },
            );
          },

          addInputPage: function (
            id,
            title,
            subtitle,
            nextPageId,
          ...
          addPageImpl: function (
            ...
            this.message.ABC_FORMS = {
              ...this.message.ABC_FORMS,
              pages: this.message.ABC_FORMS.pages.concat(page),
            };
      ...
      appleBusinesChatTimePicker: (nextIntentName) => {
        return {
          ...
          setLocation: function (
            ...
            this.message.ABC_EVENT_LOCATION = {
              title,
              latitude,
              longitude,
              radius,
          ...
          addTimeItem: function (
            ...
            this.message.ABC_EVENT_TIME_SLOTS =
              this.message.ABC_EVENT_TIME_SLOTS.concat({
          ...
          send: function () {
            resultState.say.push({
              ...
              ITEMS: [
                {
                  ...this.message,
                  ABC_SECTIONS: this.message.ABC_SECTIONS.map((m) => ({
                    title: m.title,
