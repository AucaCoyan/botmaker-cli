...
async function makeChanges(
    ...
    if (hasLocalChanges && hasIncomingChanges) {
        ...
        if (conflict) {
            console.log(
                bgRed(
                    `WARNING: ${join(wpPath, "src", status.filename)} has merge conflicts`,
                ),
            );
        } else {
            console.log(
        ...
        const newVersion: string =
            status.unPublishedCode || status.publishedCode;

        if (status.filename) {
            console.log(
                green(`${join(wpPath, "src", status.filename)} has changes`),
            );
            await writeFile(
                join(wpPath, "src", status.name),
                newVersion,
                "UTF-8",
            );
        } else {
            ...
            console.log(
                green(`${join(wpPath, "src", status.filename)} was added`),
            );
        }
    } else if (wasAdded) {
        const newVersion: string =
