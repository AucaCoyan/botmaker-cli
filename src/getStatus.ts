...
export async function getCaByNameOrPath(
    wpPath: string,
    cas: ClientAction[],
    caName: string,
) {
...
async function getRemoteStatus(token: string, id: string) {
    ...
    try {
        ...
        const { name, type, publishedCode, unPublishedCode }: ClientAction =
            JSON.parse(caResp.body);
        return {
            N: name,
            T: type,
            P: publishedCode,
...
function findRemoteStatus(
    remotesCas: ClientAction[],
    id: string,
): ClientAction {
    if (!id) return NO_REMOTE;
...
async function getStatusData(
    wpPath: string,
    ca: ClientAction,
    remoteOrToken: string | ClientAction[],
) {
