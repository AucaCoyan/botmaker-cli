...
async function main(args: string[]) {
	...
	const arrgs = yargs(args)
			...
			async (yargs) =>
				yargs
					...
					.option("port <portNumber>", {
						describe: "Change endpoint port number",
					}),
		)
		.command(["import <apiToken>", "i"], "Import a new bussiness from a token")
		.command(["set-customer <customerId>", "c"], "Load context for a customer")
		.command(["status [caName]", "s"], "Show change status")
		.command(
			["diff <caName> <code>", "d"],
		...
		.command(["pull [caName]"], "Pull incoming changes")
		.command(["new <caName>", "n"], "Create a new client action", (yargs) =>
			yargs
				.option("v", {
					alias: "vs-code",
					describe: "Open in vs-code",
		...
		.command(["push [caName]"], "Push changes in client action", (yargs) =>
			yargs.option("b", {
				alias: "publish",
				describe: "Push and publish with a single command",
			}),
		)
		.command(["publish <caName>"], "Publish changes in client action")
		.command(["rename <caName> <newName>"], "Renames the given client action")
		.demandCommand()
		.help("h")
		.alias("h", "help")
		.version("0.1.8")
		.epilog("copyright Botmaker 2022").argv;
